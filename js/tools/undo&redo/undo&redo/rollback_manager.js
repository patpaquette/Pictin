...
PICTIN.rollback_manager = new function rollback_manager(){
	...
	this.recordState = function(){
		...
		jQuery.each(PICTIN.shape_manager.shape_array, function(index, value){
			...
			var custom_class = (value.isCustom)?jQuery(value.getNode()).data("custom_shape_class"):"";
			...
			temp_shape_array.push({UID: UID, attr: attributes_array, custom_class: custom_class, events: events, isSelected: isSelected});
		...
		DEBUG_PAT.output("recording state", true);
		DEBUG_PAT.output_obj(this.state_list, "state list", true);
		DEBUG_PAT.output("Current state : " + this.state_index + " List length : " + this.state_list.length,true);
		
		DEBUG_PAT.output("Canvas rotation : " + PICTIN.current_rotation, true);
		DEBUG_PAT.output("recording state END", true);
	}
	
	this.rollback = function(){
		DEBUG_PAT.output("ROLLING BACK NOW", true);
		...
		DEBUG_PAT.output("rotation_deg : " + rotation_deg, true);
		PICTIN.current_rotation = this.state_list[this.state_index].current_rotation;
		//PICTIN.rotate(rotation_deg%360);
		DEBUG_PAT.output("current rotation : " + PICTIN.current_rotation, true);
		...
		DEBUG_PAT.output("state_index : " + this.state_index, true);
...
function copy_shape_array_attributes(target_array, attr_container_array){
	...
	jQuery.each(this._attr_container_array, function(index1, attr_container){ //looping through the array containing attribute containers
		...
		/*jQuery.each(me._target_array, function(index2, current_shape){ //looping through current shapes
			DEBUG_PAT.output("in second loop");
			DEBUG_PAT.output("current shape UID : " + jQuery(current_shape.getNode()).data("UID"));
			DEBUG_PAT.output("attr container UID : " + attr_container.UID);
			
			var _current_shape = (current_shape.isCustom)?current_shape.shape:current_shape;
		
			if(jQuery(_current_shape.getNode()).data("UID") == attr_container.UID){ 
				found_shape = true;
				me._shapes_to_keep.push(attr_container.UID);
				
				copy_attributes_to_shape(_current_shape, attr_container.attr);
		...
		if(!found_shape){ //if the shape has been deleted in the present state, we have to recreate it.
			...
			switch(attr_container.custom_class){
				...
				break;
				case "angle" : PICTIN.shape_manager.add_shape(new Angle(0, 0, 0, created_shape));
				break;
