...
PICTIN.automatic_programs_tool = new function(){
	...
	this.activate = function(){
		...
		this.pictin_color_buffer = PICTIN.current_color;
		this.chooseProgram();
		return this;
	}
	this.stop = function(){
		...
		PICTIN.current_color = this.pictin_color_buffer;
	...
	//private
	this.initK_HKA_GA = function(){
		me.taskManager = new TaskManager("K_HKA_GA");
	...
	this.startupInit = function(){
			//tools
		this.knee_hka_global_axis_preop = new APkneeHKAGlobalAxisPreop();
		this.knee_hka_global_axis_postop =  new HKA_global_axis_postop();
		this.bbl_femoral_measures_preop = new BBL_femoral_measures_preop();
		this.bbl_femoral_measures_postop = new BBL_femoral_measures_postop();
		this.bbl_skyline_views_postop = new BBL_skyline_views_postop();
	...
	this.buildProgramMenu = function(){
		...
		if(PICTIN.piam_type == 'H'){
			knee = false;
		}
		else if(PICTIN.piam_type == 'K'){
			hip = false;
		}
		...
		if(knee){
			jQuery("#automatic_tools_selection").append('<div><a href="#" id="knee_hka_global_axis_preop">KNEE: HKA global axis (Preop)</a></div>');
			jQuery("#automatic_tools_selection").append('<div><a href="#" id="knee_hka_global_axis_postop">KNEE: HKA global axis (Postop)</a></div>');
			jQuery("#automatic_tools_selection").append('<div><a href="#" id="patellar_measures_preop">KNEE: BBL femoral measures (Lat) / preop</a></div>');
			jQuery("#automatic_tools_selection").append('<div><a href="#" id="patellar_measures_postop">KNEE: BBL femoral measures (Lat) / postop</a></div>');
			jQuery("#automatic_tools_selection").append('<div><a href="#" id="BBL_skyline_views_postop">KNEE: BBL skyline views / postop</a></div>');
		}
		...
		jQuery("#knee_hka_global_axis_preop").bind("click", jQuery.proxy(this.knee_hka_global_axis_preop.initializeProgram, this.knee_hka_global_axis_preop));
		jQuery("#knee_hka_global_axis_postop").bind("click", jQuery.proxy(this.knee_hka_global_axis_postop.initializeProgram, this.knee_hka_global_axis_postop));
		jQuery("#patellar_measures_preop").bind("click", jQuery.proxy(this.bbl_femoral_measures_preop.initializeProgram, this.bbl_femoral_measures_preop));
		jQuery("#patellar_measures_postop").bind("click", jQuery.proxy(this.bbl_femoral_measures_postop.initializeProgram, this.bbl_femoral_measures_postop));
		jQuery("#BBL_skyline_views_postop").bind("click", jQuery.proxy(this.bbl_skyline_views_postop.initializeProgram, this.bbl_skyline_views_postop));
	}
}
...
function APStepInfo(func, ID, title, desc, img, color){
	this.stepFunc = func;
	this.stepID = ID;
...
//AP helper functions
AP.resultType = function(id, resultNo, label, unit, type, value, show){
	this.id = id;
...
AP.generateInfoForm = function(html, task, taskData){
	jQuery("#step_image").empty();
	jQuery("")
	jQuery("#step_desc").empty().append(html);
	...
	jQuery(document).bind("form_validated.AP", function(e){
		taskData["side"] = jQuery('input[name=side]:checked').val();
