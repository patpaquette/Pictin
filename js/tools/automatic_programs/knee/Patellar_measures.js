var BBL_femoral_measures_preop = KneeBase.extend({
	constructor: function(){
		...
		this.stepsInfos[4] = new APStepInfo(this.patellarMeasures, 4, "Step 3/4 : Tibial measures 2", "<ol start=\"2\"><li>Indicate as <strong>T'</strong> the intersection of perpendicular to <strong>C1C2</strong> passing through <strong>C3</strong> on the one hand, and anterior aspect of the tibial contour</ol>", "img/PIAM_BBL_Images/slide13.png", "ff0000");
	...
	calculateResults: function(task, taskData){
		...
		taskData["results"]["patellarMeasuresHeader"] = {header: "Step 1 : Patellar measures"};
		
		taskData["results"]["PB"] 	= new AP.resultType("PB", 1, "Patellar high \"h\" : PB", "u", "", baseUnit, true);
		taskData["results"]["AT/AP"] 	= new AP.resultType("AT/AP", 2, "Caton-Deschamps index (AT/AP)", "Absolute value", "",
														  	(getSegmentLength(getSegment2P(taskData["A"], taskData["T"]))/getSegmentLength(getSegment2P(taskData["A"], taskData["P"]))).numberFormat("0.00"), true);
		taskData["results"]["AT'/AP"]	= new AP.resultType("AT'/AP", 3, "preop BBL index (AT'/AP)", "Absolute value", "",
															(getSegmentLength(getSegment2P(taskData["A"], taskData["T'"]))/getSegmentLength(getSegment2P(taskData["A"], taskData["P"]))).numberFormat("0.00"), true);
		
		taskData["results"]["femoralMeasuresHeader"] = {header: "Step 2 : Femoral measures"};
		
		taskData["results"]["K"] 		= new AP.resultType("K", 4, "Total condylar width (F-F3): K", "u", "",
															(FF3Length/hLength*baseUnit).numberFormat("0.00"), true);
		taskData["results"]["K/h"]		= new AP.resultType("K/h", 5, "K/h Ratio", "Ansolute value", "",
															(FF3Length/hLength).numberFormat("0.00"), true);
		taskData["results"]["L"]		= new AP.resultType("L", 6, "Belleman's Posterior offset : L", "u", "",
															(LLength/hLength*baseUnit).numberFormat("0.00"), true);
		taskData["results"]["K/L"]		= new AP.resultType("K/L", 7, "K/L Ratio", "Absolute value", "",
															(FF3Length/LLength).numberFormat("0.00"), true);
		taskData["results"]["ED"]		= new AP.resultType("ED", 8, "Blum value (ED)", "u", "",
															(EDLength/hLength*baseUnit).numberFormat("0.00"), true);
		this.initTask();
		jQuery(document).trigger(task.eventFinished, [task]);
...
var BBL_femoral_measures_postop = BBL_femoral_measures_preop.extend({
	constructor: function(){
		...
		this.stepsInfos[4] = new APStepInfo(this.patellarMeasures, 4, 
											...
											"<ol start=\"2\"><li>Indicate as <strong>T'</strong> the intersection of perpendicular to <strong>C1C2</strong> passing through <strong>C3</strong> on the one hand, and anterior aspect of the tibial contour<li>Mark off the posterior-superior edge of tibial tray as <strong>T'm</strong></ol>", 
											"img/PIAM_BBL_Images/slide25.png", 
											"ff0000");
		this.stepsInfos[5] = new APStepInfo(this.patellarMeasuresContd, 5, 
											"Step 3/4 : Tibial measures 3", 
											"<ol start=\"4\"><li>Indicate the <strong>P</strong> point as the contact point between the femoral condyle and the PE insert<li>A line <strong>p</strong> parallel to <strong>TmT'm</strong> passing through this <strong>P</strong> point is automatically drawn by the program. Mark off as the <strong>Tp</strong> point the intersection of this <strong>p</strong> line and the perpendicular to this line which intersects <strong>Tm</strong></ol>",
											"img/PIAM_BBL_Images/slide26.png",
											"ff0000");
		this.stepsInfos[6] = new APStepInfo(this.calculateResults, 6, "Step 4/4 : Results", "", "img/PIAM_BBL_Images/slide27.png", "ff0000");
	...
	calculateResults: function(task, taskData){
		...
		taskData["results"]["ATm/AP"] 	= new AP.resultType("AT/AP", 3, "ATm/AP index", "Absolute value", "",
														  	(getSegmentLength(getSegment2P(taskData["A"], taskData["Tm"]))/getSegmentLength(getSegment2P(taskData["A"], taskData["P"]))).numberFormat("0.00"), true);
		taskData["results"]["ATp/AP"] 	= new AP.resultType("AT/AP", 4, "ATp/AP index", "Absolute value", "",
														  	(getSegmentLength(getSegment2P(taskData["A"], taskData["Tp"]))/getSegmentLength(getSegment2P(taskData["A"], taskData["P"]))).numberFormat("0.00"), true);
		
		taskData["results"]["femoralMeasuresHeader"] = {header: "Step 2 : Femoral measures"};
		
		taskData["results"]["K"] 		= new AP.resultType("K", 5, "Total condylar width (F-F3): K", "u", "",
															(FF3Length/hLength*baseUnit).numberFormat("0.00"), true);
		taskData["results"]["K/h"]		= new AP.resultType("K/h", 6, "K/h Ratio", "Absolute value", "",
															(FF3Length/hLength).numberFormat("0.00"), true);
		...
		taskData["results"]["tibialMeasuresHeader"] = {header: "Step 3 : Tibial measures"};
		
		taskData["results"]["C1C2^TmT'm"] = new AP.resultType("C1C2^TmT'm", 10, "Tibial post slope (C1C2^TmT'm)", "Absolute value", "",
															Math.round(toDegrees(getAngle_2Seg(getSegment2P(taskData["T'm"], taskData["Tm"]), getSegment2P(taskData["C1"], taskData["C2"])) - Math.PI/2)), true);
		this.initTask();
		jQuery(document).trigger(task.eventFinished, [task]);
	}
})